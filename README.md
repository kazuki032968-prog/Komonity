...
公式アカウントの表示名と投稿時間は、[src/constants/officialContent.ts](/Users/kazu/Documents/Komonity/src/constants/officialContent.ts) に集約しています。

練習メニュー用アカウント:

- 通常種目: `Komonity公式〇〇練習メニュー`
- 屋外種目の外練習: `Komonity公式〇〇外練習メニュー`
- 屋外種目の雨天練習: `Komonity公式〇〇雨天練習メニュー`
...
- 練習メニューは平日5時ごろに朝練メニューを投稿します。
- 練習メニューは平日12時ごろに午後練メニューを投稿します。
- 練習メニューは金曜・土曜20時ごろに土日の練習メニューを投稿します。
- 戦術は毎日7時ごろと20時ごろに投稿します。

品質ルール:

- 種目ごとの専門用語・現場課題・道具条件を必ず入れます。
- 目的、手順、注意点、失敗例、アレンジ方法を具体的に書きます。
- 同じ内容の再投稿は最低14日以上あけ、同じ順番のサイクルにしません。
- 屋外種目は外練習と雨天練習を分け、天候に合う内容にします。
- 顧問の先生がそのまま部活で使える粒度まで落とし込みます。

本番環境で公式投稿を自動化する場合は、Cloud Scheduler や管理画面などから上記ルールに沿って `timeline_posts` へ保存してください。既存のFirestore上の公式アカウント名を変更する場合は、`users` と既存投稿の `author` / `createdByUid` の整合性を保つ移行が必要です。
